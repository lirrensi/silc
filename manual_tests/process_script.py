#!/usr/bin/env python3
"""
Simple script that demonstrates the full open → type → close workflow
using the local silc CLI. It is meant to be executed directly:

    python -m tests/process_script.py

The script:
1. Finds an unused TCP port.
2. Starts the silc server on that port.
3. Runs a command to open a session.
4. Sends some input to the session.
5. Closes the session.

All output from each step is printed to the console.
"""
...
def main() -> int:
    ...
    try:
        ...
        # Give the command a moment to finish if it doesn't exit immediately
        time.sleep(0.5)
