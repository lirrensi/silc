#!/usr/bin/env python3
"""
Simple standalone script that demonstrates the complete workflow:
1. Open a silc session (run a command)
2. Type input into that session
3. Close the session

The script can be executed directly (`python -m tests/test_full_workflow.py`)
...
def main() -> int:
    ...
    # Start the silc server in the background
